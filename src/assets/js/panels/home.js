...
class Home {
    ...
    document.querySelector('.git').addEventListener('click', async() => {
        require('electron').shell.openExternal("https://tiktok.com/@th_studio")
    })}

    async websitebtn() {
        document.querySelector('.web').addEventListener('click', async() => {
            require('electron').shell.openExternal("https://twitter.com/elfo_studios")
        })}
    ...
    async initLaunch() {
        document.querySelector('.play-btn').addEventListener('click', async() => {
            ...
            launch.on('check', (e) => {
                progressBar.style.display = "block"
                document.querySelector(".text-download").innerHTML = `Instalando Modpack.. ${((DL / totDL) * 100).toFixed(0)}%`
                progressBar.value = DL;
                progressBar.max = totDL;
            ...
            launch.on('close', () => {
                ...
                info.innerHTML = `Revisando Librerias..`
